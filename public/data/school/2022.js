export const participantsNoteData = [
  {
    note: {
      en: "We received over 250 applications from 13 different Arabic speaking nationalities. Among those applications we accepted 27 applicants from 9 different countries. Among accepted, we selected four to become teaching assistants.",
      ar: "لقد تلقينا أكثر من 250 طلبًا من 13 جنسية عربية مختلفة. من بين هذه الطلبات، قبلنا 27 متقدمًا من 9 دول مختلفة. من بين القبولات، اخترنا أربعة ليصبحوا مساعدين تدريس."
    },
    image: "/assets/school/map2022.png",
    alt: "nationalities"
  }
];

export const instructorsData = {
  en: [
    {
      name: "Samar ElSheikh",
      affiliation: "Centre for Addiction and Mental Health",
      url: "https://twitter.com/SamarElsheikh",
    },
    {
      name: "Rawan AlSubaie",
      affiliation: "King Fahad Medical City/UCL",
      url: "https://twitter.com/rawryy",
    },
    {
      name: "Moataz Assem",
      affiliation: "University of Cambridge",
      url: "https://twitter.com/moatazassem",
    },
    {
      name: "Yahia H. Ali",
      affiliation: "Emory University and Georgia Tech",
      url: "https://yahiaali.com/",
    },
    {
      name: "Sarah Hosni",
      affiliation: "University of Rhode Island and Old Dominion University",
      url: "https://twitter.com/sarahihosni",
    },
    {
      name: "Mohamed Abdelhack",
      affiliation: "Krembil Centre for Neuroinformatics",
      url: "https://mabdelhack.github.io/",
    },
    {
      name: "Ramzi Halabi",
      affiliation: "Krembil Centre for Neuroinformatics",
      url: "https://www.linkedin.com/in/ramzihalabiphd/",
    },
  ],
  ar: [
    {
      name: "سمر الشيخ",
      affiliation: "مركز الإدمان والصحة العقلية",
      url: "https://twitter.com/SamarElsheikh",
    },
    {
      name: "روان السبيعي",
      affiliation: "مدينة الملك فهد الطبية/يو سي إل",
      url: "https://twitter.com/rawryy",
    },
    {
      name: "معتز عاصم",
      affiliation: "جامعة كامبريدج",
      url: "https://twitter.com/moatazassem",
    },
    {
      name: "يحيى ح. علي",
      affiliation: "جامعة إيموري ومعهد جورجيا التقني",
      url: "https://yahiaali.com/",
    },
    {
      name: "سارة حسني",
      affiliation: "جامعة رود آيلاند وجامعة أولد دومينيون",
      url: "https://twitter.com/sarahihosni",
    },
    {
      name: "محمد عبد الحق",
      affiliation: "مركز كرمبيل لعلم الأعصاب الحاسوبي",
      url: "https://mabdelhack.github.io/",
    },
    {
      name: "رمزي حلبي",
      affiliation: "مركز كرمبيل لعلم الأعصاب الحاسوبي",
      url: "https://www.linkedin.com/in/ramzihalabiphd/",
    },
  ],
};

export const welcomeVideoUrl = "https://youtu.be/o60MAFIi44M";
export const materialsData = [
  {
    topic: "Python (W1D1&2)",
    tutorialLink:
      "https://github.com/arabs-in-neuro/intro_to_comp_neuro/tree/main/python_workshop",
    videoLink:
      "https://www.youtube.com/watch?v=chinsa3C4d0&list=PLcDn4yh0rntsWolmOgZ_XC20zvxgqsbdo",
    slidesLink:
      "https://drive.google.com/file/d/1jBuwJXorfYVsQfMz7epQt6FmvrvA6CwD/view?usp=sharing",
  },
  {
    topic: "Basics of Neurosciences (W1D3)",
    videoLink: "https://youtu.be/BJyRA1iGQ3Y",
    slidesLink:
      "https://drive.google.com/file/d/1iW06BJnPvR2C_MKZNiKMg1RU-ZgVNemY/view?usp=sharing",
  },
  {
    topic: "Neural Data (W1D4)",
    videoLink: "https://youtu.be/z3Ox8F0fis0",
    slidesLink:
      "https://drive.google.com/file/d/1iW06BJnPvR2C_MKZNiKMg1RU-ZgVNemY/view?usp=sharing",
  },
  {
    topic: "Linear Algebra Tutorial 1 (W1D5)",
    tutorialLink:
      "https://colab.research.google.com/github/arabs-in-neuro/intro_to_comp_neuro/blob/main/linear_algebra/W1D5_Tutorial1.ipynb",
    videoLink: "https://youtu.be/dxv0JKHL4Fs",
    slidesLink:
      "https://drive.google.com/file/d/1_wSbDJ2D13u6AJLI_nHA0IA3o7vgmP-n/view?usp=sharing",
  },
  {
    topic: "Linear Algebra Tutorial 2 (W2D1)",
    tutorialLink:
      "https://colab.research.google.com/github/arabs-in-neuro/intro_to_comp_neuro/blob/main/linear_algebra/W2D1_Tutorial2.ipynb",
    videoLink: "https://youtu.be/rx9hc9zCtrI",
    slidesLink:
      "https://drive.google.com/file/d/1bZpKuAoqYPPOGJ5sv9V9FaEEO75phooK/view?usp=sharing",
  },
  {
    topic: "Linear Algebra Tutorial 3 (W2D2)",
    tutorialLink:
      "https://colab.research.google.com/github/arabs-in-neuro/intro_to_comp_neuro/blob/main/linear_algebra/W2D2_Tutorial3.ipynb",
    videoLink: "https://youtu.be/-iy8gQe8mMk",
    slidesLink:
      "https://drive.google.com/file/d/1yCPRiVDdqNu9SGXBivvEhQAJ44uhRnvy/view?usp=sharing",
  },
  {
    topic: "Calculus Tutorial 1 (W2D3)",
    tutorialLink:
      "https://colab.research.google.com/github/arabs-in-neuro/intro_to_comp_neuro/blob/main/calculus/W2D3_Tutorial1.ipynb",
    videoLink: "https://youtu.be/vtoq_Y1W728",
    slidesLink:
      "https://drive.google.com/file/d/12-sCKY3DgRJC0nzy5Wx1l8DE5bnd07bi/view?usp=sharing",
  },
  {
    topic: "Calculus Tutorial 2 (W2D3)",
    tutorialLink:
      "https://colab.research.google.com/github/arabs-in-neuro/intro_to_comp_neuro/blob/main/calculus/W2D3_Tutorial2.ipynb",
    videoLink: "",
    slidesLink: "",
  },
  {
    topic: "Calculus Tutorial 3 (W2D3)",
    tutorialLink:
      "https://colab.research.google.com/github/arabs-in-neuro/intro_to_comp_neuro/blob/main/calculus/W2D3_Tutorial3.ipynb",
    videoLink: "",
    slidesLink: "",
  },
  {
    topic: "Statistics Tutorial 1 (W2D4)",
    tutorialLink:
      "https://colab.research.google.com/github/arabs-in-neuro/intro_to_comp_neuro/blob/main/statistics/W2D4_Tutorial1.ipynb",
    videoLink: "https://youtu.be/KVuKH63vLeI",
    slidesLink:
      "https://drive.google.com/file/d/14-8i07u5MCAsZEPymvpxGqdisD5GrNcw/view?usp=sharing",
  },
  {
    topic: "Statistics Tutorial 2 (W2D5)",
    tutorialLink:
      "https://colab.research.google.com/github/arabs-in-neuro/intro_to_comp_neuro/blob/main/statistics/W2D5_Tutorial2.ipynb",
    videoLink: "https://youtu.be/qKSVIjnL8u8",
    slidesLink:
      "https://drive.google.com/file/d/19Bx5oaYSQ6rlhAUWcE5fUucVKDCZMgY6/view?usp=sharing",
  },
];
export const courseOutlineData = {
  en: [
    {
      nb: "1",
      days: [
        "Mon, June 13: Python Day 1",
        "Tue, June 14: Python Day 2",
        "Wed, June 15: Basics of Neurosciences",
        "Thu, June 16: Neural Data",
        "Fri, June 17: Linear Algebra Day 1 (Vectors)",
      ],
    },
    {
      nb: "2",
      days: [
        "Mon, June 20: Linear Algebra Day 2 (Matrices)",
        "Tue, June 21: Linear Algebra Day 3 (Eigen Values and Eigen Vectors)",
        "Wed, June 22: Calculus (Differentiation and Integration)",
        "Thu, June 23: Statistics Day 1 (Probability)",
        "Fri, June 24: Statistics Day 2 (Inference)",
      ],
    },
  ],
  ar: [
    {
      nb: "1",
      days: [
        "الإثنين، 13 يونيو: يوم البايثون 1",
        "الثلاثاء، 14 يونيو: يوم البايثون 2",
        "الأربعاء، 15 يونيو: أساسيات علم الأعصاب",
        "الخميس، 16 يونيو: البيانات العصبية",
        "الجمعة، 17 يونيو: الجبر الخطي اليوم 1 (المتجهات)",
      ],
    },
    {
      nb: "2",
      days: [
        "الإثنين، 20 يونيو: الجبر الخطي اليوم 2 (المصفوفات)",
        "الثلاثاء، 21 يونيو: الجبر الخطي اليوم 3 (قيم ومتجهات ذاتية)",
        "الأربعاء، 22 يونيو: الاحتمالات",
        "الخميس، 23 يونيو: الإحصاء اليوم 1 (الاحتمالات)",
        "الجمعة، 24 يونيو: الإحصاء اليوم 2 (الاستدلال)",
      ],
    },
  ],
};

export const calendarUrl =
  "https://calendar.google.com/calendar/embed?src=5nvg7s13rchqsi8g0nvo5tgbj0%40group.calendar.google.com";

export const courseDaysData = {
  en: [
    {
      title: "Python Days",
      description:
        "They are whole workshop days. The day in total is composed of 5 hours of tutorials including a 1-hour break in the middle. The first day includes a welcome video to be watched offline before joining.",
      schedule: [
        { time: "0:00-2:00", lecture: "Tutorials" },
        { time: "2:00-3:00", lecture: "Break" },
        { time: "3:00-5:00", lecture: "Tutorials" },
      ],
    },
    {
      title: "Neuroscience Days",
      description:
        "Neuroscience days are composed of lectures that are recorded to be watched offline and a live Q&A session with the instructors.",
      schedule: [{ time: "1:00-2:00", lecture: "Live Q&A" }],
    },
    {
      title: "Linear Algebra, Calculus, Statistics Days",
      description:
        "These days will include a lecture to be watched offline before as well as tutorials to be conducted live. The last day contains also a live outro conclusion of the course.",
      schedule: [
        { time: "0:00-2:00", lecture: "Tutorials" },
        { time: "2:00-3:00", lecture: "Break" },
        { time: "3:00-5:00", lecture: "Tutorials" },
      ],
    },
  ],
  ar: [
    {
      title: "أيام بايثون",
      description:
        "هي أيام عمل كاملة لورش العمل. يتألف اليوم بالكامل من 5 ساعات من الدروس التعليمية بما في ذلك استراحة لمدة ساعة واحدة في المنتصف. يتضمن اليوم الأول فيديو ترحيبي لمشاهدته دون اتصال قبل الانضمام.",
      schedule: [
        { time: "0:00-2:00", lecture: "الدروس التعليمية" },
        { time: "2:00-3:00", lecture: "استراحة" },
        { time: "3:00-5:00", lecture: "الدروس التعليمية" },
      ],
    },
    {
      title: "أيام علم الأعصاب",
      description:
        "تتألف أيام علم الأعصاب من محاضرات يتم تسجيلها للمشاهدة دون اتصال وجلسة أسئلة وأجوبة مباشرة مع المحاضرين.",
      schedule: [{ time: "1:00-2:00", lecture: "الأسئلة والأجوبة المباشرة" }],
    },
    {
      title: "أيام الجبر الخطي، التفاضل والتكامل، الإحصاء",
      description:
        "تتضمن هذه الأيام محاضرة للمشاهدة دون اتصال قبل ذلك بالإضافة إلى دروس تعليمية تُجرى مباشرة. يتضمن اليوم الأخير أيضًا ختام مباشر للدورة التدريبية.",
      schedule: [
        { time: "0:00-2:00", lecture: "الدروس التعليمية" },
        { time: "2:00-3:00", lecture: "استراحة" },
        { time: "3:00-5:00", lecture: "الدروس التعليمية" },
      ],
    },
  ],
};


export const collabsData = [
  {
    type: {
      en: "Proudly supported by",
      ar: "بفخر يدعمها"
    },
    items: [
      { image: "/assets/school/ibro.jpg", alt: "Ibro" },
    ]
  },
  {
    type: {
      en: "Partners",
      ar: "الشركاء"
    },
    items: [
      { image: "/assets/school/egyptscholars.png", alt: "Egypt Scholars" }
    ]
  }
];